...
<body>
    <div class="header">
        <h1>INVOICE</h1>
        <p class="text-muted mb-0">Booking Reference: #{{ $booking->id }}</p>
        <p class="text-muted">Issued Date: {{ now()->format('F d, Y') }}</p>
    </div>
    ...
    <div class="booking-info">
        <div class="info-box">
            ...
            <p class="mb-0"><strong>Customer Since:</strong> {{ $booking->user->created_at->format('M Y') }}</p>
        </div>

        <div class="info-box">
            <h3>Booking Details</h3>
            <p class="mb-2"><strong>Booking Date:</strong> {{ $booking->created_at->format('M d, Y') }}</p>
            <p class="mb-2">
            ...
            <p class="mb-0"><strong>Invoice #:</strong> INV-{{ $booking->id }}-{{ date('Y') }}</p>
        </div>
    </div>
    ...
    <div class="date-range">
        ...
        <p class="mb-0">
            <strong>{{ \Carbon\Carbon::parse($booking->start_date)->format('M d, Y') }}</strong>
            to
            <strong>{{ \Carbon\Carbon::parse($booking->end_date)->format('M d, Y') }}</strong>
    ...
    <div style="background: #f0f9ff; padding: 20px; border-radius: 12px; margin-bottom: 25px; border-left: 4px solid #0ea5e9;">
        ...
        <p class="mb-2"><strong>Van:</strong> {{ $booking->van->name }}</p>
        <p class="mb-2"><strong>Model:</strong> {{ $booking->van->model ?? 'Standard Model' }}</p>
        <p class="mb-0"><strong>Seating Capacity:</strong> {{ $booking->van->seats ?? 'N/A' }} passengers</p>
    </div>
    ...
    <table class="table">
        ...
        <tbody>
            <tr>
                <td>
                    <strong>Van Rental - {{ $booking->van->name }}</strong><br>
                    <small class="text-muted">
                        {{ $booking->van->model ?? 'Standard Model' }} |
                        {{ $booking->total_days }} day rental period
                    </small>
                </td>
                <td>{{ $booking->total_days }} days</td>
                <td>${{ number_format($booking->van->price_per_day, 2) }}/day</td>
                <td>${{ number_format($booking->van->price_per_day * $booking->total_days, 2) }}</td>
            </tr>
            ...
            <tr>
                <td>
                    ...
                    <small class="text-muted">Location: {{ $booking->pickup_location }}</small>
                </td>
                <td>1 service</td>
                <td>Complimentary</td>
                <td>$0.00</td>
            ...
            <tr>
                <td>
                    ...
                    <small class="text-muted">Location: {{ $booking->dropoff_location }}</small>
                </td>
                <td>1 service</td>
                <td>Complimentary</td>
                <td>$0.00</td>
            ...
            <tr style="background: #f1f5f9;">
                ...
                <td style="font-weight: bold;">${{ number_format($booking->van->price_per_day * $booking->total_days, 2) }}</td>
            </tr>

            <tr>
                <td colspan="3" style="text-align: right;">Tax (10%)</td>
                <td>${{ number_format($booking->van->price_per_day * $booking->total_days * 0.1, 2) }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="total-section">
        ...
        <p class="total-amount">${{ number_format($booking->total_price, 2) }}</p>
